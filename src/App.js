 ...
 // 컴포넌트가 마운트될 때 fetchLatestStats 함수를 호출
  ...
  useEffect(() => {
    const fetchDrawNumbers = async () => {
      try {
        const response = await axios.get('10.17.4.113:8000/reactun/api/weeks');
  ...
  useEffect(() => {
    ...
    if (selectedDrawNumber) {
      axios.get(`10.17.4.113:8000/reactun/api/lotto-stats/${selectedDrawNumber}`)
        .then(response => {
          const { data } = response;
  ...
  //     try {
  //       const response = await axios.get('10.17.4.113:8000/reactun/api/latest-stats');
...
const generateNumbersWithSpecific = () => {
  ...
  const generationWeek = new Date().toISOString().slice(0, 10); // YYYY-MM-DD 형식의 주차 정보
    ...
    axios.post('10.17.4.113:8000/reactun/api/lotto-numbers', {
...
// console.log('@@setLatestStatsa@@', latestStats);
  ...
  // console.log(' ㅇ__ㅇ '); 
    ...
    axios.post('10.17.4.113:8000/reactun/api/lotto-numbers', { generatedNumbers: newNumbers,generationWeek })
    .then(response => {
      console.log('Data inserted successfully', response.data);
    })
    .catch(error => {
  ...
  //   axios.get('10.17.4.113:8000/reactun/scrape')
  //     .then(response => {
  //       console.log(response.data);
  ...
  //   axios.post('10.17.4.113:8000/reactun/api/upload', formData)
