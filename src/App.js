 ...
 // 컴포넌트가 마운트될 때 fetchLatestStats 함수를 호출
  ...
  useEffect(() => {
    const fetchDrawNumbers = async () => {
      try {
        const response = await axios.get('https://reactun-untab.koyeb.app:8000/api/weeks');
  ...
  useEffect(() => {
    ...
    if (selectedDrawNumber) {
      axios.get(`https://reactun-untab.koyeb.app:8000/api/lotto-stats/${selectedDrawNumber}`)
        .then(response => {
          const { data } = response;
  ...
  //     try {
  //       const response = await axios.get('https://reactun-untab.koyeb.app:8000/api/latest-stats');
...
const generateNumbersWithSpecific = () => {
  ...
  const generationWeek = new Date().toISOString().slice(0, 10); // YYYY-MM-DD 형식의 주차 정보
    ...
    axios.post('https://reactun-untab.koyeb.app:8000/api/lotto-numbers', {
...
// console.log('@@setLatestStatsa@@', latestStats);
  ...
  // console.log(' ㅇ__ㅇ '); 
    ...
    axios.post('https://reactun-untab.koyeb.app:8000/api/lotto-numbers', { generatedNumbers: newNumbers,generationWeek })
    .then(response => {
      console.log('Data inserted successfully', response.data);
    })
    .catch(error => {
  ...
  //   axios.get('https://reactun-untab.koyeb.app:8000/scrape')
  //     .then(response => {
  //       console.log(response.data);
  ...
  //   axios.post('https://reactun-untab.koyeb.app:8000/api/upload', formData)
