 ...
 // 컴포넌트가 마운트될 때 fetchLatestStats 함수를 호출
  ...
  useEffect(() => {
    const fetchDrawNumbers = async () => {
      try {
        const response = await axios.get('${process.env.REACT_APP_API_BASE_URL}/api/weeks');
  ...
  useEffect(() => {
    ...
    if (selectedDrawNumber) {
      axios.get(`${process.env.REACT_APP_API_BASE_URL}/api/lotto-stats/${selectedDrawNumber}`)
        .then(response => {
          const { data } = response;
  ...
  //     try {
  //       const response = await axios.get('${process.env.REACT_APP_API_BASE_URL}/api/latest-stats');
...
const generateNumbersWithSpecific = () => {
  ...
  const generationWeek = new Date().toISOString().slice(0, 10); // YYYY-MM-DD 형식의 주차 정보
    ...
    axios.post('${process.env.REACT_APP_API_BASE_URL}/api/lotto-numbers', {
...
// console.log('@@setLatestStatsa@@', latestStats);
  ...
  // console.log(' ㅇ__ㅇ '); 
    ...
    axios.post('${process.env.REACT_APP_API_BASE_URL}/api/lotto-numbers', { generatedNumbers: newNumbers,generationWeek })
    .then(response => {
      console.log('Data inserted successfully', response.data);
    })
    .catch(error => {
  ...
  //   axios.get('${process.env.REACT_APP_API_BASE_URL}/scrape')
  //     .then(response => {
  //       console.log(response.data);
  ...
  //   axios.post('${process.env.REACT_APP_API_BASE_URL}/api/upload', formData)
